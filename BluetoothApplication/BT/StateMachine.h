...
public:
    ...
    public:
        ...
        std::shared_ptr<StateMachine> mSm;
        long mTime;
        int mWhat;
        std::string mInfo;
        std::shared_ptr<IState> mState;
        std::shared_ptr<IState> mOrgState;
        std::shared_ptr<IState> mDstState;
        ...
        LogRec
            ...
            std::shared_ptr<IState> transToState
            )
        {
         ...
         */
        std::shared_ptr<IState> getDestState()
        {
            return mDstState;
        }
         ...
         */
        std::shared_ptr<IState> getOriginalState()
        {
            return mOrgState;
        }
...
private:
    ...
    private:
        ...
        std::vector<LogRec> mLogRecVector;
        int mMaxSize = DEFAULT_SIZE;
        int mOldestIndex = 0;
        int mCount = 0;
        bool mLogOnlyTransitions = false;
        std::mutex mMutex;
        ...
        void setLogOnlyTransitions( bool enable )
        {
            std::lock_guard<std::mutex> locker( mMutex );
            mLogOnlyTransitions = enable;
        }

        bool logOnlyTransitions()
        {
            std::lock_guard<std::mutex> locker( mMutex );
        ...
        LogRec get( int index )
        {
            ...
            {
                return mLogRecVector.at( nextIndex );
            }
        }
            ...
            )
        {
            ...
            {
                mLogRecVector.emplace_back( sm, msg, messageInfo, state, orgState, transToState );
            }
            else
            {
                LogRec pmi = mLogRecVector.at( mOldestIndex );
                mOldestIndex += 1;
    ...
    private:
        ...
        */
        class StateInfo
        {
            ...
            std::shared_ptr<StateInfo> parentStateInfo;
        ...
        public:
            ...
            std::string toString()
            {
                return "state="+state->getName()+",active="+( active ? "true" : "false" )+",parent="
                    +( ( parentStateInfo ) ? parentStateInfo->state->getName() : "null" );
            }
        };
    ...
    private:
        ...
        //static final Object mSmHandlerObj = new Object();

        /** The current message */
        std::shared_ptr<Message> mMsg;
        ...
        std::shared_ptr<HaltingState> mHaltingState;

        /** State used when state machine is quitting */
        std::shared_ptr<QuittingState> mQuittingState;

        /** Reference to the StateMachine */
        std::shared_ptr<StateMachine> mSm;

        /** The map of all of the states in the state machine */
        std::map<std::shared_ptr<IState>, StateInfo> mStateInfo;
         ...
         */
        bool mTransitionInProgress = false;

        /** The list of deferred messages */
        std::vector<std::shared_ptr<Message>> mDeferredMessages;
     ...
     public:
        ...
        void handleMessage( std::shared_ptr<Message> msg )
        {
            if( !mHasQuit )
            {
                if( mSm && msg->what != SM_INIT_CMD && msg->what != SM_QUIT_CMD )
                {
